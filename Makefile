...
INCS= -I$(LUA_INC)
CFLAGS= -O3 $(WARN) $(INCS) $(DEFS) `pkg-config --cflags --libs gtk+-2.0 libnotify` 
CXXFLAGS= -O3 $(WARN) $(INCS) $(DEFS)
CC= gcc
...
EXTRA_DIST=Makefile application-zip.svg document-print.svg example.lua \
	gnome-mime-application-pdf.svg system-shutdown.svg
